...
def getSentences(request):
    try:
        ...
        dataArray = appAPIUtils.getSentencesData(sentenceLevel = sentenceLevel, sentenceTopic = sentenceTopic, sentenceClass = sentenceClass, aboutWord = aboutWord, sentenceMinLength = sentenceMinLength, sentenceMaxLength = sentenceMaxLength, sentenceRanking = sentenceRanking, sentenceRankingLocking = sentenceRankingLocking, dataLimit = dataLimit)
        # 初始化回傳值
        response = {
            'apiStatus' : 'success',
			'apiMessage' : 'success',
...
def getPhoneticExercisesSentencesByWordSet(request):
    try:
        ...
        # 整理從資料庫得到的資料
        wordData = appAPIUtils.getWordLearningData(learningClassification = learningClassification, learningPhase = learningPhase)

        sentencesData = []
        for word in wordData:
            dataArray = appAPIUtils.getSentencesData(sentenceRankingLocking = word['wordRanking'], dataLimit = '3')
...
def checkSentences(request):
    try:
        ...
        # 根據IPA相似度給予評價
        scoreComment = appAPIUtils.getScoreComment(ipaTextSimilarity, correctCombo)
        ...
        for errorWord in questionErrorWordArray:
            ...
            for ipaAboutWord in sample(rhymesWordArray, min(len(rhymesWordArray),3)):
                pronunciationData = appAPIUtils.getWordPronunciationData(ipaAboutWord)
        ...
        # IPA發音檢驗
        checkPronunciation = appAPIUtils.checkPronunciation(questionIPAText, answerIPAText)
...
def checkSentences2(request):
    try:
        ...
        # IPA音節檢驗
        pronunciationCompare = appAPIUtils.textCompare(questionIPAText, answerIPAText)
